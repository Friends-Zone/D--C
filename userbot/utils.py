...
from userbot import CMD_LIST, SUDO_LIST, CMD_HELP, CMD_HELP_BOT, COMMAND_HAND_LER
import re
import logging
import inspect
import math
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                darkreg = f"^{Config.COMMAND_HAND_LER}"
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                darkreg = f"^{Config.SUDO_COMMAND_HAND_LER}"
...
class CmdHelp:
    """
    The class I kanged to better generate command aids.
    """
    ...
    def __init__(self, file: str, official: bool = True, file_name: str = None):
        ...
        self.FILE_NAME = file_name if file_name is not None else f"{file}.py"
        self.COMMANDS = {}
        self.FILE_AUTHOR = ""
        self.WARNING = ""
        self.INFO = ""
    ...
    def get_result(self):
        ...
        for command in self.COMMANDS:
            ...
            if command["example"] is None:
                ...
                result += f"**💬 Details :** `{command['usage']}`\n"
                result += (
                    f"**⌨️ Example :** `{COMMAND_HAND_LER[:1]}{command['example']}`\n\n"
                )
        return result
